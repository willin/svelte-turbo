...
export class GitHubStrategy<User> extends OAuth2Strategy<User, GitHubProfile, GitHubExtraParams> {
  ...
  protected async getAccessToken(response: Response): Promise<{
    ...
    const accessTokenExpiresIn = parseExpiresIn(data.get('expires_in'));
    const refreshTokenExpiresIn = parseExpiresIn(data.get('refresh_token_expires_in'));
